...
Client.prototype.uploadFile = function(params) {
  ...
  function startPuttingObject() {
    ...
    if (localFileStat.size >= self.multipartUploadThreshold) {
      ...
      openFile(beginMultipartUpload);
    } else {
      openFile(beginSingleUpload);
    }
    ...
    function beginMultipartUpload() {
      startMultipartUpload(multipartUploadSize);
    }

    function beginSingleUpload() {
  ...
  function startMultipartUpload(multipartUploadSize) {
    ...
    function checkDoMoreWork() {
      if (fatalError || noMoreWork) return;
      if (cursor >= localFileStat.size) {
        if (partsCompleted >= parts.length) {
          noMoreWork = true;
  ...
  function tryPuttingObject(cb) {
    self.s3Pend.go(function(pendCb) {
      ...
      s3Params.Body = inStream;
      if (localFileStat.multipartETag && localFileStat.multipartETag.digest) {
        s3Params.ContentMD5 = localFileStat.multipartETag.digest.toString('base64');
      }
