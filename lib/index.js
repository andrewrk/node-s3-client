...
Client.prototype.uploadFd = function(params) {
  var self = this;
  ...
  // if they didn't provide fd then they must plan to call start with the fd
  if (fd) setImmediate(statFd);

  return uploader;

  function handleStart(_fd) {
    fd = _fd;
    setImmediate(statFd);
  }
  ...
  function statFd() {
    localFileSlicer = new FdSlicer(fd, {autoClose: autoClose});
...
Client.prototype.uploadFile = function(userParams) {
  var self = this;

  var params = extend({}, userParams);
  params.autoClose = true;
  var uploader = self.uploadFd(params);

  fs.open(params.localFile, 'r', function(err, fd) {
    if (err) {
      uploader.emit('error', err);
      return;
    }
