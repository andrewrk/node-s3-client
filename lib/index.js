  ...
  function startPuttingObject() {
    if (localFileStat.size >= self.multipartUploadThreshold) {
      ...
      fs.open(localFile, 'r', function(err, fd) {
        ...
        uploader.emit('fdSlicer', localFileSlicer);
      ...
      function tryUploadPart(cb) {
        ...
        self.s3Pend.go(function(pendCb) {
          ...
          counter.on('progress', function() {
            if (fatalError) return;
            var delta = counter.bytes - prevBytes;
            prevBytes = counter.bytes;
          ...
          pend.go(function(cb) {
            counter.on('finish', function() {
              ...
              prevBytes = counter.bytes;
              uploader.progressAmount += delta;
              uploader.progressTotal += (end - start) - counter.bytes;
              uploader.emit('progress');
              cb();
            });
          });
          ...
          inStream.pipe(hash);
          self.s3.uploadPart(s3Params, function(err, data) {
            pendCb();
            if (fatalError || errorOccurred) return;
            if (err) {
              errorOccurred = true;
            ...
            pend.wait(function() {
              if (fatalError) return;
              if (!compareETag(data.ETag, hashDigest)) {
                errorOccurred = true;
...
function compareETag(eTag, md5Buffer) {
  return cleanETag(eTag) === md5Buffer.toString('hex');
}
